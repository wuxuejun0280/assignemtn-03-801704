...
## 2. Batch analysis
For further batch analysis of the data outputed by streaming analytics, we can calculate the average pickup per location at that specific time in a year. First, the stream ingestion component will add the amount of pick up at that specific time. So the database should have a collection for each year in which the amount of pick up per location at that 1 hour interval per year will be stored and updated whenever the collection receive a new record. Then the batch analysis could calculate the average pick up per hour for all the collections, which represents data fom different years. Also since the workload for stream analysis is too light, probably we can add information about the total fare amount and total trip distance into the output record. Then the batch analysis could also calculate the average miles traveled and the average income per ride at that time interval in a year. 

## 3. The critical condition
![](https://github.com/wuxuejun0280/assignemtn-03-801704/blob/master/reports/critical_condition.png "design")

Generally I don't think it is necessary to have a critical condition in this case. With this dataset, it is easier for batch analytic app to run periodically to calculate the average of the pick up per location. But if there must be a critical condition, I think it could be when the amount of time interval is updated reaches certain limits, the batch stream analytics will be triggered. So the stream ingestion will post to a condition queue the time interval that's updated. Then the computing service will keep track of the amount of time interval received from condition queue. If the amount reaches certain limit, or one interval is updated for certain amount of time, the critical condition is triggered. The computing service will start the batch analytic of historical data. 

## 4. scale stream analytics
Here to scale the stream analytics for different customers, we need to allow customer to upload their own stream analytics app. The computing service will provide api for customer to upload the app and set the config, such as the message queue uri and the parallelism level, of their analytics app. The computing service will start the job automatically once the app is uploaded. Of course it should also provide api to start and stop their analytic app and provice log to help user allocate the error encountered. 

## 5. About deliver guarrantee
I think in this case it is hard to achieve exactly-once delivery. In flink we can use check point to enforce exactly once delivery. However, the client cannot ensure the message will be delivered. On one hand, the taxi doesn't necessarily have the good network condition and some message may be brokena and lost in the transmitting. On the other hand, some taxi requires to upload the data manually which is highly unreliable. So to achieve exatly-once delivery, we need to ensure the delivery from the client side, which seems to be impossible to achieve right now. 
